...
The cache exists because resolution asks for the model configuration dozens of
times, and building one reads the checkpoint's metadata off disk -- or, for
weights behind an object-store URI, out of a directory the launcher pulled.

Two things make the key subtle:

- resolution itself moves the record's `model_path` (the GGUF and ModelScope
  handlers declare a local path for a Hub reference), so a configuration built
  before them describes a different checkpoint and has to be rebuilt;
- `ModelConfig` moves *its own* `model_path` to the local pull directory when
  the weights sit behind a URI, so the built object's field stops matching the
  record it was built from while still describing the same checkpoint.

Reading the second as the first rebuilds on every call, and on a resolved record
the rebuild cannot store its result -- the read-only guard refuses the write --
so every `--load-format runai_streamer` launch died in the scheduler.
"""
...
class TestTheModelConfigCache(CustomTestCase):
    ...
    def test_a_configuration_that_repoints_itself_stays_cached(self):
        """The object-store shape, which is what the nightly launched."""
    ...
    def test_the_cache_refills_on_a_resolved_record(self):
        ...
        The record is read-only once resolution has finished, so a rebuild that
        went through the field would raise there instead of answering.
        """
        first_checkpoint = self._checkpoint()
        second_checkpoint = self._checkpoint()
    ...
    def test_a_supplied_configuration_is_handed_back(self):
        """A fixture's stand-in leaves no key behind, so nothing invalidates
        it."""
